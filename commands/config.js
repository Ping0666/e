...
const { mainprefix, defaultjoinmessage, defaultleavemessage, color, defaultmultiplyer } = yaml.load(
...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (content.toLowerCase() === "multiplyer") {
      let prefixembed = new Discord.MessageEmbed()
           .setColor(`${embed || color}`)
        .setTitle(`**multiplyer**`)
        .setDescription(
          `This config is currently set.
      Use \`\`\`${prefix}config multiplyer <value>\`\`\` to change it.
      `
        )
        .setFooter(message.guild.name, client.user.displayAvatarURL());
      let newmulti = args[1];
      ...
      let changedprefix = new Discord.MessageEmbed()
        .setTitle(`**multiplyer Updated**`)
             .setColor(`${embed || color}`)
        .setDescription(
          "successfully updated :)"
        )
        .setFooter(message.guild.name, client.user.displayAvatarURL());
      db.delete(`multiplyer_${message.guild.id}`);
      db.set(`multiplyer_${message.guild.id}`, newmulti);
      return message.channel.send(changedprefix);
    }

    if (content.toLowerCase() === "cookie") {
      let prefixembed = new Discord.MessageEmbed()
           .setColor(`${embed || color}`)
        .setTitle(`**Roblox Cookie**`)
        .setDescription(
          `This config is currently set.
      Use \`\`\`${prefix}config cookie <value>\`\`\` to change it.
      `
        )
        .setFooter(message.guild.name, client.user.displayAvatarURL());
      let newcookie = args[1];
      ...
      let changedprefix = new Discord.MessageEmbed()
        ...
        .setFooter(message.guild.name, client.user.displayAvatarURL());
      db.delete(`cookie_${message.guild.id}`);
      db.set(`cookie_${message.guild.id}`, newcookie);
      return message.channel.send(changedprefix);
    }
    ...
    if (content.toLowerCase() === "show") {
      ...
      let multiplyer = db.get(`multiplyer_${message.guild.id}`);
      if (multiplyer === null) multiplyer = defaultmultiplyer;
      ...
      let guildconfig = new Discord.MessageEmbed()
        ...
        .addField(`Multiplyer`, multiplyer, true)
        .addField(`Roblox Cookie`, "hidden, sorry", true)
